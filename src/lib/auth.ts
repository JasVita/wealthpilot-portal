...
export async function verifyCreds(email: string, pw: string): Promise<VerifyCredsResult> {
  ...
  const env = envUsers.find((x) => x.email.toLowerCase() === email.toLowerCase());
  if (!env) return { ok: false, reason: "no_user" } as const;

  const ok = await bcrypt.compare(pw, env.password);
  return ok ? { ok: true, user: { id: env.id, email: env.email } } : { ok: false, reason: "bad_pw" };
}
...
export async function createUser(email: string, pw: string) {
  const pool = getPool();
  const { rows: exists } = await pool.query("SELECT 1 FROM public.users WHERE username = $1", [email]);
